...
${diff > 0 ? '🚀 Du läufst besser als letztes Jahr!' : '⚠️ Achte auf deine Leistung!'}`;
    ...
    generatePersonalizedInsights() {
        ...
        if (weekly && weekly.worked !== undefined && weekly.worked !== null) {
            const workedNum = typeof weekly.worked === 'number' ? weekly.worked : parseFloat(weekly.worked) || 0;
            const expectedNum = typeof weekly.expected === 'number' ? weekly.expected : parseFloat(weekly.expected) || 0;
            
            insight += `📈 **Diese Woche:** ${workedNum.toFixed(1)}h von ${expectedNum.toFixed(1)}h erwartet\n`;
            if (weekly.percentage !== undefined) {
                const percNum = typeof weekly.percentage === 'number' ? weekly.percentage : parseFloat(weekly.percentage) || 0;
                insight += `   → ${percNum.toFixed(0)}% deines Solls ✓\n`;
            }
            insight += '\n';
        }
        ...
        if (trends && trends.trend) {
            ...
            if (trends.average !== undefined) {
                const avgNum = typeof trends.average === 'number' ? trends.average : parseFloat(trends.average) || 0;
                insight += `   Durchschnitt: ${avgNum.toFixed(1)} Stunden\n`;
            }
            insight += '\n';
        }
        ...
        if (weekly && weekly.diff !== undefined) {
            const diffNum = typeof weekly.diff === 'number' ? weekly.diff : parseFloat(weekly.diff) || 0;
            if (diffNum < -5) {
                insight += `• Dein Saldo ist negativ (${diffNum.toFixed(1)}h). Plane nächste Woche intensiver!\n`;
            } else if (diffNum > 10) {
                insight += `• Du schuldest dir ${diffNum.toFixed(1)}h Freizeit! Gönne dir eine Pause 😎\n`;
            }
        }
